...
{{--                                // 3) Fallback to model accessor (if any images resolved there)--}}
...
{{--@push('scripts')--}}
{{--    <script>--}}
{{--        // Listen for variant image selections from the Add-to-Cart component and swap gallery image accordingly--}}
{{--        document.addEventListener('livewire:init', function () {--}}
{{--            const PD_STATE = {--}}
{{--                captured: false,--}}
{{--                originalSlidesHtml: null,--}}
{{--                originalThumbsHtml: null,--}}
{{--                originalStickySrc: null,--}}
{{--            };--}}

{{--            function captureOriginalGallery() {--}}
{{--                if (PD_STATE.captured) return;--}}
{{--                const mainWrapper = document.querySelector('.tf-product-media-main .swiper-wrapper');--}}
{{--                const thumbsWrapper = document.querySelector('.tf-product-media-thumbs .swiper-wrapper');--}}
{{--                const stickyImg = document.querySelector('.tf-sticky-atc-img img');--}}

{{--                if (mainWrapper) {--}}
{{--                    PD_STATE.originalSlidesHtml = mainWrapper.innerHTML;--}}
{{--                }--}}
{{--                if (thumbsWrapper) {--}}
{{--                    PD_STATE.originalThumbsHtml = thumbsWrapper.innerHTML;--}}
{{--                }--}}
{{--                if (stickyImg) {--}}
{{--                    PD_STATE.originalStickySrc = stickyImg.getAttribute('data-src') || stickyImg.getAttribute('src');--}}
{{--                }--}}
{{--                PD_STATE.captured = true;--}}
{{--            }--}}

{{--            function destroySwiper(swiperEl) {--}}
{{--                if (swiperEl && swiperEl.swiper) {--}}
{{--                    try { swiperEl.swiper.destroy(true, true); } catch (e) { /* noop */ }--}}
{{--                }--}}
{{--            }--}}

{{--            function swapToVariant(url) {--}}
{{--                const mainEl = document.querySelector('.tf-product-media-main');--}}
{{--                const mainWrapper = mainEl?.querySelector('.swiper-wrapper');--}}
{{--                if (!mainEl || !mainWrapper) return;--}}

{{--                // Ensure we have the original gallery saved--}}
{{--                captureOriginalGallery();--}}

{{--                // Destroy existing swiper to avoid state issues--}}
{{--                destroySwiper(mainEl);--}}

{{--                // Replace with single slide for the variant image--}}
{{--                const safeUrl = String(url);--}}
{{--                mainWrapper.innerHTML = `--}}
{{--                    <div class="swiper-slide">--}}
{{--                        <a href="${safeUrl}" target="_blank" class="item" data-pswp-width="1200" data-pswp-height="1200">--}}
{{--                            <img class="tf-image-zoom" src="${safeUrl}" data-src="${safeUrl}" data-zoom="${safeUrl}" alt="Variant image">--}}
{{--                        </a>--}}
{{--                    </div>`;--}}

{{--                // Hide thumbs and navigation when only one slide--}}
{{--                const thumbsEl = document.querySelector('.tf-product-media-thumbs');--}}
{{--                if (thumbsEl) thumbsEl.style.display = 'none';--}}
{{--                document.querySelectorAll('.thumbs-next, .thumbs-prev').forEach(el => el.style.display = 'none');--}}

{{--                // Update sticky image thumbnail--}}
{{--                const stickyImg = document.querySelector('.tf-sticky-atc-img img');--}}
{{--                if (stickyImg) {--}}
{{--                    stickyImg.src = safeUrl;--}}
{{--                    stickyImg.setAttribute('data-src', safeUrl);--}}
{{--                }--}}
{{--            }--}}
...
{{--                const mainEl = document.querySelector('.tf-product-media-main');--}}
{{--                const mainWrapper = mainEl?.querySelector('.swiper-wrapper');--}}
{{--                if (!mainEl || !mainWrapper || !PD_STATE.originalSlidesHtml) return;--}}

{{--                // Destroy variant swiper state (if any)--}}
{{--                destroySwiper(mainEl);--}}

{{--                // Restore HTML--}}
{{--                mainWrapper.innerHTML = PD_STATE.originalSlidesHtml;--}}
{{--                const thumbsEl = document.querySelector('.tf-product-media-thumbs');--}}
{{--                if (thumbsEl) {--}}
{{--                    thumbsEl.style.display = '';--}}
{{--                    const tw = thumbsEl.querySelector('.swiper-wrapper');--}}
{{--                    if (tw && PD_STATE.originalThumbsHtml !== null) tw.innerHTML = PD_STATE.originalThumbsHtml;--}}
{{--                }--}}
{{--                document.querySelectorAll('.thumbs-next, .thumbs-prev').forEach(el => el.style.display = '');--}}

{{--                // Re-initialize gallery swiper after a brief delay to allow DOM update--}}
{{--                setTimeout(() => { try { if (typeof initGallerySwiper === 'function') initGallerySwiper(); } catch (e) {} }, 60);--}}

{{--                // Restore sticky image--}}
{{--                const stickyImg = document.querySelector('.tf-sticky-atc-img img');--}}
{{--                if (stickyImg && PD_STATE.originalStickySrc) {--}}
{{--                    stickyImg.src = PD_STATE.originalStickySrc;--}}
{{--                    stickyImg.setAttribute('data-src', PD_STATE.originalStickySrc);--}}
{{--                }--}}
{{--            }--}}

{{--            // Capture initial gallery state once DOM is ready--}}
{{--            if (document.readyState === 'loading') {--}}
{{--                document.addEventListener('DOMContentLoaded', captureOriginalGallery, { once: true });--}}
{{--            } else {--}}
{{--                captureOriginalGallery();--}}
{{--            }--}}

{{--            // Robust event payload handling (supports object, string, or array forms)--}}
{{--            Livewire.on('product:variantImage', function () {--}}
{{--                let url = null;--}}
...
{{--                        url = first;--}}
{{--                    } else if (first && typeof first === 'object' && 'imageUrl' in first) {--}}
{{--                        url = first.imageUrl;--}}
{{--                    }--}}
{{--                } else if (args.length > 1) {--}}
{{--                    // Some Livewire versions pass each named param as an arg--}}
{{--                    const found = args.find(a => a && typeof a === 'object' && 'imageUrl' in a);--}}
{{--                    if (found) url = found.imageUrl;--}}
{{--                }--}}
