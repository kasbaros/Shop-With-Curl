                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                                <div class="tf-product-media-wrap sticky-top">
                                    @php
                                        // Use product's gallery or featured_image with ImageStorageHelper
                                        $gallery = [];
                                        if (is_array($product->gallery) && count($product->gallery) > 0) {
                                            foreach ($product->gallery as $imagePath) {
                                                $gallery[] = [
                                                    'thumb' => \App\Helpers\ImageStorageHelper::url($imagePath),
                                                    'large' => \App\Helpers\ImageStorageHelper::url($imagePath),
                                                    'original' => \App\Helpers\ImageStorageHelper::url($imagePath),
                                                ];
                                            }
                                        } elseif ($product->featured_image) {
                                            $gallery[] = [
                                                'thumb' => \App\Helpers\ImageStorageHelper::url($product->featured_image),
                                                'large' => \App\Helpers\ImageStorageHelper::url($product->featured_image),
                                                'original' => \App\Helpers\ImageStorageHelper::url($product->featured_image),
                                            ];
                                        } else {
